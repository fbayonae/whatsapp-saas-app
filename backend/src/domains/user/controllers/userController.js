...
const getAllSessionsByUser = async (req, res) => {
    ...
    try {
        const sessions = await userService.getSessionsUserFromDB( parseInt(id));
        res.json(sessions);
    } catch (error) {
...
const updateUser = async (req, res) => {
    ...
    try {
        ...
        const updated = await userService.updateUserFromDB(id, updateData);
...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        const existing = await userService.getUserByEmailFromDB(email);
        if (existing) {
        ...
        const user = await userService.createUserFromDB({ name, email, passwordHash, role });
