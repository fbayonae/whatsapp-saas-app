...
const syncTemplates = async (req, res) => {
    ...
    try {
        ...
        for (const tpl of templates) {
            const savedTpl = await templateService.saveTemplateToDB(prisma, tpl);
            if (tpl.components?.length) {
                for (const comp of tpl.components) {
                    await templateService.saveComponentToDB(prisma, comp, savedTpl.id);
                }
            }
...
const generateTemplatePayload = async (req, res) => {
    ...
    try {
        const template = await templateService.getTemplateByIdFromDB(prisma, templateId);
        if (!template) {
...
const getTemplates = async (req, res) => {
    const prisma = getPrismaClient(req.user.tenantId);
    try {
        const templates = await templateService.getTemplatesFromDB(prisma);
        res.json(templates);
    } catch (error) {
        console.error('❌ Error obteniendo plantillas:', error);
        res.status(500).json({ error: 'Error al obtener las plantillas' });
...
const deleteTemplate = async (req, res) => {
    ...
    try {
        ...
        const template = await templateService.getTemplateByIdFromDB(prisma, template_id_meta);

        if (!template) {
            return res.status(404).json({ error: 'Plantilla no encontrada' });
        }

        const response = await templateService.deleteTemplateFromDB(prisma, template_id_meta);
        if (!response) {
...
const createTemplate = async (req, res) => {
    ...
    try {
        ...
        const saveTemplate = await templateService.saveTemplateToDB(prisma, template);
        if (template.components?.length) {
            for (const comp of template.components) {
                await templateService.saveComponentToDB(prisma, comp, response.id);
            }
        }
